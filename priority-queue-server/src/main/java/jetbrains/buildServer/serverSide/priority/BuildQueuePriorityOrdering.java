...
public final class BuildQueuePriorityOrdering implements BuildQueueOrderingStrategy {
  ...
  // Next 3 maps use SQueuedBuild.getItemId() as keys, because SQueuedBuild
  ...
  public synchronized List<SQueuedBuild> addBuilds(@NotNull final List<SQueuedBuild> itemsToAdd,
    ...
    if (!TeamCityProperties.getBooleanOrTrue("teamcity.buildQueue.priorityOrdering.enabled"))
  ...
  public synchronized void restoreQueue(@NotNull final List<SQueuedBuild> queuedBuilds) {
    try {
      ...
      for (SQueuedBuild item : queuedBuilds) {
        int buildPropertyPriority = 0;
        try {
          buildPropertyPriority = Integer
              .valueOf(item.getBuildPromotion().getBuildParameters().getOrDefault("teamcity.buildqueue.priority", "0"));
        } catch (NumberFormatException e) {
        }
        int buildTypePriority = getCurrentBuildTypePriority(item) + buildPropertyPriority;
  ...
  private void addNewItems(@NotNull final List<SQueuedBuild> itemsToAdd,
      @NotNull final List<SQueuedBuild> currentQueueItems) {
    Set<String> buildIds = getIds(currentQueueItems);
    for (SQueuedBuild item : itemsToAdd) {
      if (buildIds.contains(item.getItemId())) {
        myLogger
            .info("The current queue items already contains the build " + item + ", don't add it to the priority order");
        continue;
      }
      int buildPropertyPriority = 0;
      try {
  ...
  private void logItemAdded(final List<SQueuedBuild> items, final SQueuedBuild item, final int position,
      final double weight) {
    final int defaultPosition = items.size() - 1; // default position is in the end of the queue, minus 1 because item
                                                  // already added
    if (myLogger.isDebugEnabled()) {
      myLogger.debug("Current item priorities: " + myItemWeights + ", new item " + item + " with weight " + weight
      ...
      if (position != defaultPosition) {
        ...
        if (previousItem != null) {
          ...
          myLogger.info(String.format(Locale.ENGLISH,
              "New item %s with weight %.2f inserted at position %d instead of %d, between items %s (weight %.2f) and %s (weight %.2f)",
              item.toString(), weight, position, defaultPosition, previousItem, previousItemWeight, nextItem,
              nextItemWeight));
        } else {
          Double nextItemWeight = getItemWeight(nextItem.getItemId());
          myLogger.info(String.format(Locale.ENGLISH,
              "New item %s with weight %.2f inserted at position %d instead of %d, before item %s (weight %.2f)",
              item.toString(), weight, position, defaultPosition, nextItem, nextItemWeight));
        }
      } else {
        myLogger.info(String.format(Locale.ENGLISH,
            "New item %s with weight %.2f inserted at the default position %d in the end of the queue",
   ...
   * When this method returns myItemWeights, myMovedItemsPriorities, myLastResult
   * and myPrioritiesOnTheInsertMoment
  ...
  /**
   * Recalculate queued builds weights according to movements in the build queue
   * and theirs wait times.
   * 
  ...
  /**
   * Recalculate priorities of moved queued builds. Each moved queued build get
   * priority of
   ...
   * @return queued item estimate duration or DEFAULT_DURATION if estimate
   ...
   * @return priority of item's build type or if item was moved it's recomputed
   *         priority
   */
   ...
   * 
   * @param item queued item
   * @return priority of item build type on the moment when it was added to the
   *         queue
   */
  private int getBuildTypePriorityOnTheInsertMoment(SQueuedBuild item) {
    ...
    if (priorityOnTheMomentOfInsert != null) {
      ...
      myLogger
          .error("Item " + item.toString() + " was added, but it's build type priority on the insert moment is lost");
      return 0;
    }
  }
  ...
  public String toString() {
    return String.format(
        "BuildQueuePriorityOrdering state: myItemWeights=%s, myMovedItemsPriorities=%s, myPrioritiesOnTheInsertMoment=%s, myLastResult=%s.",
